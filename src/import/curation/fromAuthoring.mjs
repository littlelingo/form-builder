...
export function createRecipeFromAuthoringForm(form, options = {}) {
  ...
  const recipe = {
    ...
    fields: rows.map(({ chapter, page, component, provenance }) => ({
      ...
      ...(chapter.type ? { chapterType: chapter.type } : {}),
      ...(chapter.options ? { chapterOptions: chapter.options } : {}),
      ...(chapter.itemNameLabel ? { itemNameLabel: chapter.itemNameLabel } : {}),
      ...(chapter.sectionIntro ? { sectionIntro: chapter.sectionIntro } : {}),
      pageId: page.id,
